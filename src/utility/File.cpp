...
bool File::open(const std::string &file_path, char open_mode, bool binary, bool truncate) {
	if (m_is_open) {
		m_file.close();
		std::printf("File::Closing previous file and opening new");
	}
	...
	if (m_open_mode == 'a')
		...
		std::printf("File::Error: wrong file open flag provided");
		return false;
	}
	...
	if (!m_is_open)
		std::printf("File::Error:Unable to open file %s", file_path.c_str());
...
void File::close() {
	if (m_is_open)
        ...
        std::printf("File::File already closed");
}
